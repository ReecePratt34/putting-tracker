        ...
        <div key={group} style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {SHOT_SHAPES.filter(s => s.group === group).map(shape => (
              <Pill key={shape.key} active={selectedShapes.find(s => s.key === shape.key)} onClick={() => toggleShape(shape.key) || setSelectedShapes(prev => {
                const exists = prev.find(s => s.key === shape.key);
                return exists ? prev.filter(s => s.key !== shape.key) : [...prev, SHOT_SHAPES.find(s => s.key === shape.key)];
              })}>
                {shape.label}
              </Pill>
            ))}
